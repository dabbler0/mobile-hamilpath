...
`localStorage` key (`settings.ts`'s `loadMusicVolume`/`saveMusicVolume`,
`DEFAULT_MUSIC_VOLUME` 0.5 — a little lower than sfx's own 0.6 default,
since it's meant to sit underneath everything else) and its own master gain
node entirely separate from `audio/sfx.ts`'s — turning one down never
